...
class Copter(TrackingObject):
    ...
    def __compute_id(self, leds):
        ...
        id_leds_count ={}
        for led in id_leds:
          id_leds_count[led.color] = 0
        for led in id_leds:
          id_leds_count[led.color] += 1
        if len(id_leds_count) > 1:
            rospy.loginfo("Too many id leds found for copter at (%s, %s)", self.x, self.y)
            self.id = max(id_leds_count, key=id_leds_count.get)
            return True
        if len(id_leds_count) == 0:
            rospy.loginfo("No id led found for copter at (%s, %s)", self.x, self.y)
            self.id = 0
            return False
        self.id = id_leds_count.keys()[0]
        return True
...
class CopterModel(TrackingModel):
    """
        FINken-III-Max Tracking Model containing parameter sets and create function to spawn Copters
    """


    def __init__(self):
        super(CopterModel, self).__init__("FINken-III-Max tracking model")
        ...
        self.led_params[LedParams.colors] = ["blue", "green", "red"]
        self.led_params[LedParams.hs_values] = {"blue":((0.66, 0.5), (0.15, 0.5)),
                                                "green":((0.33, 0.5), (0.15, 0.5)),
                                                "red":((0.0, 0.5), (0.15, 0.5))}
                                                #"white":((0.5, 0.25), (0.5, 0.25))}

    def create(self):
        return Copter(self.roi_params)
