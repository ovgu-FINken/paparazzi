...
from CopterModel import CopterModel
...
def resize_pubs(name):
    if not name in pubs:
        pubs[name]=rospy.Publisher('/copter/'+name, TaggedPose2D, queue_size=1)
...
if __name__ == '__main__':
    ...
    tracker = Tracker(CopterModel(), homography)
    subscriber = rospy.Subscriber('/camera/image', Image, callback, queue_size=1)
    try:
        rospy.spin()
